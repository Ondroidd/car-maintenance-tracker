...
def print_fuel_eco(total_liters, total_fuel_cost, avg_price_per_liter, avg_fuel_eco):
    ...
    fuel_stats = Table(title="Fuel statistics", box=box.SIMPLE_HEAD, row_styles=["", "dim"])

    fuel_stats.add_column("Tot. fuel cons.", justify="center", style="cyan", no_wrap=True)
    fuel_stats.add_column("Tot. fuel cost (CZK)", justify="center", style="cyan", no_wrap=True)
    fuel_stats.add_column("Avg. cost/l (CZK)", justify="center", style="cyan", no_wrap=True)
    fuel_stats.add_column("Avg. fuel eco", justify="center", style="cyan", no_wrap=True)
...
def show_fuel_stats(data):
    ...
    if total_liters == 0:
        print("\nTotal liters is 0, cannot compute averages.")
        return
    ...
    if len(sorted_by_mileage) < 2:
        print("\nNot enough fuel entries to compute fuel economy.\n")
    ...
    if not economies:
        print("\nCannot compute fuel economy. Not enough entries.\n")
        print_fuel_eco(total_liters, total_fuel_cost, avg_price_per_liter, "N/A")
        return
...
def show_maintenance_stats(data):
    ...
    maintenance_stats = Table(title="Maintenance statistics", box=box.SIMPLE_HEAD, row_styles=["", "dim"])

    maintenance_stats.add_column("Tot. entries", justify="center", style="cyan", no_wrap=True)
    maintenance_stats.add_column("Tot. Maintenance Cost (CZK)", justify="center", style="cyan", no_wrap=True)
    maintenance_stats.add_column("Avg. Cost/Entry (CZK)", justify="center", style="cyan", no_wrap=True)
